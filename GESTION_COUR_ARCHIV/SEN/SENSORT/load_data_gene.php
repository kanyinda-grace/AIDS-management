...
join agent on courrier_sort.mat_agent=agent.mat_agent
 ORDER BY courrier_sort.num_lettre_sort LIMIT $start, $per_page");
  ...
  $users=$db->prepare("select count(num_lettre_sort) as tota from courrier_sort ");
  $users->execute(array('num_lettre_sort'));
...
<div align='center' style='font-size:18px;font-weight:bold;color:red;'>LISTE DES COURRIERS SORTANTS DE TOUS LES SERVICES</div> 
...
$query_pag_num = "SELECT COUNT(*) AS count  FROM courrier_sort ";
