...
left join agent on courrier_sort.mat_agent=agent.mat_agent  where service.code_service='DAFF' 


UNION ALL
SELECT courrier_ent.num_lettre_ent,service.code_service,service.libelle_service,courrier_sort.num_lettre_sort,
courrier_sort.jour_sort,courrier_sort.mois_sort,courrier_sort.annee_sort
,courrier_ent.objet_ent,courrier_ent.observation_ent, courrier_ent.code_sexe,courrier_ent.nom_ent,courrier_ent.postnom_ent,courrier_ent.prenom_ent,courrier_ent.entreprise_ent,courrier_ent.email_ent,
 courrier_ent.telephone_ent,courrier_ent.libelle_type_document_ent,courrier_ent.commentaire
 from courrier_sort
...
right join courrier_ent on courrier_sort.num_lettre_ent=courrier_ent.num_lettre_ent 
left join service on courrier_ent.code_service=service.code_service 
left join agent on courrier_sort.mat_agent=agent.mat_agent  where service.code_service='DAFL' 
...
right join courrier_ent on courrier_sort.num_lettre_ent=courrier_ent.num_lettre_ent 
left join service on courrier_ent.code_service=service.code_service 
left join agent on courrier_sort.mat_agent=agent.mat_agent  where service.code_service='DAFR' 
...
<div align='center' style='font-size:18px;font-weight:bold;color:red;'>LISTE DE TOUS LES COURRIERS  DU DAF</div> 
...
<table border='0' cellspacing='1' width='100%' align='center' class='table table-bordered' style='border-width:1px;border-style:solid;border-color:black;width:70%' >
            <thead  class=''  >
                <tr style='background-color:#0F9DE8;' style='border-width:1px;border-style:solid;border-color:black;'>
                    <th style='border-width:1px;border-style:solid;border-color:black;'><div style='color:white;font-size:17px'>NUMERO LETTRE ENTRANTE</div></th>
                    <th style='border-width:1px;border-style:solid;border-color:black;'><div style='color:white;font-size:17px'>NUMERO LETTRE SORTANTE</div></th>
                     <th style='border-width:1px;border-style:solid;border-color:black;'><div style='color:white;font-size:17px'>TYPE DOCUMENT</div></th>
          ...
          <th style='border-width:1px;border-style:solid;border-color:black;'><div style='color:white;font-size:17px'>EXPEDITEUR</div></th>
  ...
  $users=$db->prepare("SELECT COUNT(*) AS count  FROM courrier_ent ");
$data=$users->fetch();
$ttgg=$data['num_lettre_ent'];
echo $ttgg ;
